...
fun App() {
    ...
    Column(
        ...
        UseCaseCard(
            ...
            currentValue = "Status: ${env.powerInfo.status.name} (Battery: ${env.powerInfo.batteryLevel?.let {
                "${(it * 100).toInt()}%"
            } ?: "N/A"})",
            suggestion = if (env.powerInfo.status == PowerState.LOW_POWER ||
                env.powerInfo.batteryLevel ?: 1.0f < 0.2f
            ) {
                "Consider disabling or reducing animations"
            } else {
                "Normal power — animations OK"
            },
            isHighlight = env.powerInfo.status == PowerState.LOW_POWER || env.powerInfo.batteryLevel ?: 1.0f < 0.2f,
        )
        ...
        UseCaseCard(
            ...
            currentValue = "Type: ${env.networkInfo.type.name} (Metered: ${env.networkInfo.isMetered})",
            suggestion = if (env.networkInfo.isMetered) {
                "Use lower resolution or defer heavy downloads"
            } else if (!env.networkInfo.isConnected) {
                "Show offline UI"
            } else {
                "Unmetered & Connected — full quality OK"
            },
            isHighlight = env.networkInfo.isMetered || !env.networkInfo.isConnected,
        )
        ...
        UseCaseCard(
            ...
            currentValue = "Status: ${caps.biometric.status.name} (${caps.biometric.type.name})",
            suggestion = if (caps.biometric.status == BiometricStatus.READY) {
                "You can show biometric login"
            } else {
                "Hide or disable biometric option"
            },
            isHighlight = caps.biometric.status == BiometricStatus.READY,
        )
        ...
        UseCaseCard(
            ...
            currentValue = "Live: net=${env.networkInfo.type}, " +
                "pwr=${env.powerInfo.status}, " +
                "dev=${env.deviceInfo.deviceClass}",
        ...
        Card(
            ...
            Column(modifier = Modifier.padding(16.dp)) {
                ...
                Text(
                    "Device: ${env.deviceInfo.deviceClass} " +
                        "(${env.deviceInfo.manufacturer} ${env.deviceInfo.model}) • " +
                        "OS: ${env.deviceInfo.osName} ${env.deviceInfo.osVersion} • " +
                        "Language: ${env.localeInfo.language} • " +
                        "Country: ${env.localeInfo.country} • " +
                        "24hr: ${if (env.localeInfo.is24HourFormat) "Yes" else "No"} • " +
                        "TZ: ${env.timezoneInfo.id.ifEmpty { "—" }} " +
                        "(${env.timezoneInfo.displayName})"
                )
            }
        }
    }
